...
let handler = async (m, { conn, text }) => {
 if (!text) return conn.reply(m.chat,`🐯 Enter the link of a Spotify Track, Playlist or Album.`, m) 
 let isSpotifyUrl = text.match(/^(https:\/\/open\.spotify\.com\/(album|track|playlist)\/[a-zA-Z0-9]+)/i);
 if (!isSpotifyUrl && !text) return conn.reply(m.chat, `🐯 Enter the link of any Spotify Track, Playlist or Album.`, m)
...
let txt = `*乂  S P O T I F Y  -  D O W N L O A D*\n\n`
    ...
    txt += `	✩   *Published* : ${album.metadata.releaseDate}\n`   
    txt += `	✩   *Tracks totales* : ${album.trackList.length}\n\n`   
    txt += `*- ↻ The audios are being sent, wait a moment, I'm slow. . .*`
...
let txt = `*乂  S P O T I F Y  -  D O W N L O A D*\n\n`
    ...
    txt += `	✩   *Duration* : ${track.duration}\n`
    txt += `	✩   *Album* : ${track.album.name}\n`                 
    txt += `	✩   *Published* : ${track.album.releasedDate}\n\n`   
    txt += `*- ↻ The audios are being sent, wait a moment, I'm slow. . .*`
await conn.sendFile(m.chat, img, 'thumbnail.jpg', txt, m)
...
let txt = `*乂  S P O T I F Y  -  D O W N L O A D*\n\n`
    txt += `	✩   *Title* : ${searchTrack.title}\n`
    txt += `	✩   *Artist* : ${searchTrack.artists}\n`
    txt += `	✩   *Duration* : ${searchTrack.duration}\n`
    txt += `	✩   *Album* : ${searchTrack.album.name}\n`                 
    txt += `	✩   *Published* : ${searchTrack.album.releasedDate}\n\n`   
    txt += `*- ↻ The audios are being sent, wait a moment, I'm slow. . .*`
await conn.sendFile(m.chat, img, 'thumbnail.jpg', txt, m)
